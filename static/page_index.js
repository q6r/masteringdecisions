...
function buildTemplate() {
    ...
    var nav_section = $([
        ...
        '<li><a onclick="buildEditUsers()"><i class="glyphicon glyphicon-user"></i> MyShit</a></li>',
...
/**** User Stuff ****/
function buildEditUsers() {
    $('title').html('Users');
    clearContent();

    $('<strong><i class="glyphicon glyphicon-cog"></i> Edit Users</strong><hr/>').appendTo('#content');
    ...
    $('<div id="userList">').appendTo(wrapper);
    $('#userList').hide();
    $('<hr/>').appendTo(wrapper);
...
function updateUserList() {
    ...
    get_text("/persons", function(results) {
        ...
        table.append('<tr><th>First</th><th>Last</th><th>Email</th><th></th></tr>');

        if (results["persons"].length < 1) $('#altList').hide();
        else $('#userList').show();

        for (var i in results["persons"]) {
            a = results["persons"][i];
            table.append('<tr><td>' + a['name_first'] + '</td><td>' + a['name_last'] + '</td><td>' + a['email'] + '</td><td>' + '<div style="width:45px; float:right;"><a onclick="showEditUser(' + a['person_id'] + ');"><span class="glyphicon glyphicon-pencil text-Primary"></span></a>' + '<a onclick="deleteUser();"><span class="glyphicon glyphicon-trash text-Danger" style="margin-left:10px;"></span></a></div></td></tr>');
        }
...
function showEditUser(userID) {
    ...
    $('#userEdit').html("");
    $('<h3>Edit User</h3>').appendTo('#userEdit');
    ...
    $('<div class="form-group">').append(
            $('<label for="userEdit">Email</label>'),
            $('<input type="test" />').addClass('form-control')
            .attr('name', 'userEmail')
            .attr('placeholder', 'Alternative Description')
            .attr('id', 'userEmail'))
        .appendTo('#userEdit');

    $('<button>').addClass('btn btn-primary').attr('onclick', 'showAddUser();').text('Back to Add New User').attr('style', 'float: left').appendTo('#userEdit');
    $('<button>').addClass('btn btn-primary').attr('onclick', 'showEditUser();').text('Update User').attr('style', 'float: right').appendTo('#userEdit');
    $('<div>').addClass('clearfix').appendTo('#userEdit'); //added to fix display issue

    get_text("/person/" + userID + "/info", function(result) {
        $('#userID').val(result['person_id']);
        $('#userEmail').val(result['email']);
        $('#userFirst').val(result['name_first']);
        $('#userLast').val(result['name_last']);
    });
}
